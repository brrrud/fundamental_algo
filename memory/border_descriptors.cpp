...
{
    return sizeof(size_t) + sizeof(logger *) + sizeof(memory::allocation_mode) + sizeof(memory *) + sizeof(void *);
}
...
size_t border_descriptors_memory::get_service_part_occupied_block_size() const
{
...
{
    size_t serv_occup_block_size = get_service_part_occupied_block_size();
...
{
    auto alloc_method = *reinterpret_cast<memory::allocation_mode *>(reinterpret_cast<unsigned char *>(_all_memory) + sizeof(size_t) + sizeof(logger *));
    ...
    while (true)
    {
        ...
        {
            if (alloc_method == memory::allocation_mode::first_fit ||
                alloc_method == memory::allocation_mode::best_fit && (target_block == nullptr || target_block_size > block_size) ||
                alloc_method == memory::allocation_mode::worst_fit && (target_block == nullptr || target_block_size < block_size))
            {
...
void border_descriptors_memory::deallocate(void *const target_to_dealloc) const
{
    auto true_target_to_dealloc = reinterpret_cast<void *>(reinterpret_cast<unsigned char *>(const_cast<void *>(target_to_dealloc)) - get_service_part_occupied_block_size());
...
border_descriptors_memory::border_descriptors_memory(logger *log, memory *allocator, size_t size, memory::allocation_mode method)
{
    ...
    *log_block = log;
    auto mode_block = reinterpret_cast<memory::allocation_mode *>(log_block + 1);
    *mode_block = method;
...
border_descriptors_memory::~border_descriptors_memory()
{
    ...
    auto *outer_allocator = *reinterpret_cast<memory **>(reinterpret_cast<unsigned char *>(_all_memory) + sizeof(size_t) + sizeof(logger *) + sizeof(memory::allocation_mode));
