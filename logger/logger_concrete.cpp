...
logger const *logger_concrete::log(const std::string &to_log,logger::severity severity) const{
    ...
    for (auto & logger_stream : _logger_streams)
    {
        ...
        {
            std::cout << "[" << curr_severity << "]" << "[" <<ltm->tm_mday <<ltm->tm_hour<<":"<<ltm->tm_min<<":"<<ltm->tm_sec << "] " << to_log << std::endl;
        }
        else
        {
            
            (*logger_stream.second.first) << "[" << curr_severity << "]" << "[" <<ltm->tm_mday << ltm->tm_hour<<":"<<ltm->tm_min<<":"<<ltm->tm_sec << "] " << to_log << std::endl;
        }
    }
