#ifndef FUNDAMENTAL_ALGO_AVL_H
#define FUNDAMENTAL_ALGO_AVL_H
#ifndef AVL_TREE_H
#define AVL_TREE_H
...
protected:
    ...
    private:
        [[nodiscard]] size_t get_node_size() const override;

        void call_constructor_node(typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *mem) const override;

        void initialize_node_additional_data(
                typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *mem) const override;

        void inject_additional_data(typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *from,
                                    typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *to) const override;

    public:
        explicit avl_insertion_template_method(avl_tree<tkey, tvalue, tkey_comparer> *tree);
    };

protected:
    class avl_removing_template_method final
            : public binary_search_tree<tkey, tvalue, tkey_comparer>::removing_template_method {
    private:
        avl_tree<tkey, tvalue, tkey_comparer> *_tree;

    protected:
        void after_remove_inner(tkey const &key,
                                typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *&subtree_root_address,
...
private:
    ...
    void fix_height(typename binary_search_tree<tkey, tvalue, tkey_comparer>::node **subtree_address);

    size_t balance_factor(typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *subtree_address);

public:
    avl_tree(avl_tree<tkey, tvalue, tkey_comparer> const &other);

    avl_tree(avl_tree<tkey, tvalue, tkey_comparer> &&other) noexcept;

    avl_tree &operator=(avl_tree<tkey, tvalue, tkey_comparer> const &other);

    avl_tree &operator=(avl_tree<tkey, tvalue, tkey_comparer> &&other) noexcept;
};
...
void avl_tree<tkey, tvalue, tkey_comparer>::fix_height(
    ...
    if (*subtree_address != nullptr)
        reinterpret_cast<avl_tree<tkey, tvalue, tkey_comparer>::avl_node *>(*subtree_address)->height =
...
avl_tree<tkey, tvalue, tkey_comparer>::avl_insertion_template_method::avl_insertion_template_method(
        avl_tree<tkey, tvalue, tkey_comparer> *tree) : _tree(tree),
                                                       binary_search_tree<tkey, tvalue, tkey_comparer>::insertion_template_method(
                                                               tree) {
}
...
avl_tree<tkey, tvalue, tkey_comparer>::avl_removing_template_method::avl_removing_template_method(
        avl_tree<tkey, tvalue, tkey_comparer> *tree) : _tree(tree),
                                                       binary_search_tree<tkey, tvalue, tkey_comparer>::removing_template_method(
                                                               tree) {
}
...
avl_tree<tkey, tvalue, tkey_comparer>::avl_tree(memory *allocator, logger *logger)
        : binary_search_tree<tkey, tvalue, tkey_comparer>(new avl_insertion_template_method(this),
                                                          new typename binary_search_tree<tkey, tvalue, tkey_comparer>::reading_template_method(
                                                                  this),
...
avl_tree<tkey, tvalue, tkey_comparer>::avl_tree(avl_tree<tkey, tvalue, tkey_comparer> const &other)
        : avl_tree<tkey, tvalue, tkey_comparer>(other._allocator, other._logger) {
