#ifndef FUNDAMENTAL_ALGO_SPLAY_H
#define FUNDAMENTAL_ALGO_SPLAY_H
...
protected:
    class splay_insertion_template_method final
...
protected:
    class splay_removing_template_method final
    ...
    public:
        explicit splay_removing_template_method(splay_tree<tkey, tvalue, tkey_comparer> *tree);
    };

protected:
    void splay(typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *&subtree_root_address,
               std::stack<typename binary_search_tree<tkey, tvalue, tkey_comparer>::node **> &path_to_subtree_root_exclusive);
...
public:
    splay_tree(splay_tree<tkey, tvalue, tkey_comparer> const &other);

    splay_tree(splay_tree<tkey, tvalue, tkey_comparer> &&other) noexcept;

    splay_tree &operator=(splay_tree<tkey, tvalue, tkey_comparer> const &other);

    splay_tree &operator=(splay_tree<tkey, tvalue, tkey_comparer> &&other) noexcept;
};
...
void splay_tree<tkey, tvalue, tkey_comparer>::splay_insertion_template_method::insert_inner(
    ...
    if (subtree_root_address == nullptr) {
        subtree_root_address = reinterpret_cast<typename binary_search_tree<tkey, tvalue, tkey_comparer>::node *>(this->allocate_with_guard(
                get_node_size()));
        ...
        insert_inner(key, std::move(value), res_functor > 0 ? subtree_root_address->right_subtree_address
...
tvalue splay_tree<tkey, tvalue, tkey_comparer>::splay_removing_template_method::remove_inner(
    ...
    while (*current_node != nullptr) {
        ...
        if (_tree->_comparator(key, (*current_node)->key) == 0) {
            ...
            current_node = &(_tree->_comparator(key, (*current_node)->key) > 0 ? (*current_node)->right_subtree_address
...
#endif //FUNDAMENTAL_ALGO_SPLAY_H
